...
def categorical(df):
    ...
    """
    categorical_var=list(set(df.columns)-set(df._get_numeric_data().columns))
...
def instances_based_condition(df,col1,val1,col2,val2):
    ...
    """
    instance=df.loc[(df[col2]==val2) & (df[col1]>val1)]
    return instance
...
def agg_values_ina_month(df,date_col,agg_col, agg):
    ...
    """
    #df['date_col_mon']=pd.to_datetime(df[date_col].dt.to_period('M'))
    df['date_col_mon']=pd.DatetimeIndex(df[date_col]).month
    aggregated_value=pd.pivot_table(df,index='date_col_mon',values=agg_col,aggfunc='mean')
...
def group_values(df,col1,agg1):
   ...
   Keyword arguments:
    ...
    """
    grouping=df.groupby('Weather').agg({x:agg1 for x in col1})
    return grouping
...
print(categorical(weather_2012))
print(numerical(weather_2012))
num=numerical(weather_2012)
...
print(clear(weather_2012,'Weather','Cloudy'))
...
wind_speed_35_vis_25=instances_based_condition(weather_2012,'Wind Spd (km/h)',35,'Visibility (km)',25)
...
print(agg_values_ina_month(weather_2012,'Date/Time','Temp (C)','mean').head())
...
mean_weather=group_values(weather_2012,num,'mean')
...
#print(convert(weather_2012,'Temp (C)').head(3))
